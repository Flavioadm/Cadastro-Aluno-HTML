...
CREATE TABLE IF NOT EXISTS `mydb`.`nomeAluno` (
...
CREATE TABLE IF NOT EXISTS `mydb`.`cadastroTurma` (
  `id` INT(11) NOT NULL AUTO_INCREMENT,
...
CREATE TABLE IF NOT EXISTS `mydb`.`periodo` (
  ...
  CONSTRAINT `fk_nomeAluno`
    ...
    REFERENCES `mydb`.`nomeAluno` (`id`)
    ON DELETE RESTRICT
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_cadastroTurma`
    FOREIGN KEY (`id_cadastroTurma`)
    REFERENCES `mydb`.`cadastroTurma` (`id`)
    ON DELETE RESTRICT
    ON UPDATE NO ACTION)
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8;
